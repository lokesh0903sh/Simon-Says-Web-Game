...
const useFriendsStore = create((set, get) => ({
  ...
  fetchPendingRequests: async () => {
    ...
    try {
      ...
      const pendingRequests = response.requests || response;
  ...
  fetchSentRequests: async () => {
    ...
    try {
      ...
      const sentRequests = response.requests || response;
  ...
  acceptFriendRequest: async (requestId) => {
    ...
    try {
      ...
      const pendingRequests = pendingResponse.requests || pendingResponse;
  ...
  fetchFriendsLeaderboard: async (period = 'all', page = 1, limit = 50) => {
    set({ loading: true, error: null });
    try {
      const response = await friendsAPI.getFriendsLeaderboard(period, page, limit);
      const friendsLeaderboard = response.leaderboard || response;
      set({ friendsLeaderboard, loading: false });
      return response;
    } catch (error) {
